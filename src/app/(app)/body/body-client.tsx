...
import {
  SectionCards, ChartArea, Section, EmptyState, Spinner, type KpiCard,
...
export function BodyClient({ bodyRecords, userId }: Props) {
  ...
  const oldWeight = sorted.filter(r => r.weight_kg != null && new Date(r.recorded_at) <= oneMonthAgo).at(-1)
  const oldFat = sorted.filter(r => r.body_fat_pct != null && new Date(r.recorded_at) <= oneMonthAgo).at(-1)

  const kpiCards: KpiCard[] = [
    ...(latestWeight?.weight_kg != null ? [{
      title: '体重',
      value: `${latestWeight.weight_kg}kg`,
      description: oldWeight?.weight_kg != null ? `1ヶ月前: ${oldWeight.weight_kg}kg` : '比較データなし',
      icon: <Scale className="w-4 h-4" />,
      trend: oldWeight?.weight_kg != null ? (() => {
        const diff = latestWeight.weight_kg! - oldWeight.weight_kg!
        const sign = diff > 0 ? '+' : ''
        return { direction: (diff < 0 ? 'up' : diff > 0 ? 'down' : 'neutral') as 'up' | 'down' | 'neutral', value: `${sign}${diff.toFixed(1)}kg` }
      })() : undefined,
    }] : []),
    ...(latestFat?.body_fat_pct != null ? [{
      title: '体脂肪率',
      value: `${latestFat.body_fat_pct}%`,
      description: oldFat?.body_fat_pct != null ? `1ヶ月前: ${oldFat.body_fat_pct}%` : '比較データなし',
      trend: oldFat?.body_fat_pct != null ? (() => {
        const diff = latestFat.body_fat_pct! - oldFat.body_fat_pct!
        const sign = diff > 0 ? '+' : ''
        return { direction: (diff < 0 ? 'up' : diff > 0 ? 'down' : 'neutral') as 'up' | 'down' | 'neutral', value: `${sign}${diff.toFixed(1)}%` }
      })() : undefined,
    }] : []),
  ]
        ...
        <div className="flex flex-wrap gap-3 items-end">
          <div className="space-y-1.5">
            ...
            <Input type="date" value={dateInput} onChange={e => setDateInput(e.target.value)}
              className="h-9 w-36" max={todayStr()} />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs text-muted-foreground">体重 (kg)</Label>
            <Input type="number" placeholder="例: 72.0" value={weightInput}
              onChange={e => setWeightInput(e.target.value)} inputMode="decimal" className="h-9 w-28" />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs text-muted-foreground">体脂肪率 (%)</Label>
            <Input type="number" placeholder="例: 22.0" value={bodyFatInput}
              onChange={e => setBodyFatInput(e.target.value)} inputMode="decimal" className="h-9 w-28" />
          </div>
          <Button onClick={handleSubmit} disabled={loading} size="sm" className="gap-1.5 h-9">
            {loading ? <><Spinner size="sm" />記録中...</> : <><Plus className="w-3.5 h-3.5" />記録する</>}
          </Button>
        </div>
      </div>
      ...
      {bodyRecords.length === 0 ? (
        ...
        <>
          ...
          <div className="space-y-6">
            ...
            <Section title="記録" description={`${bodyRecords.length}件`} variant="bordered">
                    ...
                    {editingId === r.id ? (
                      <div className="py-3 space-y-3">
                        <div className="flex flex-wrap gap-3">
                          ...
                          <div className="space-y-1">
                            ...
                            <Input type="number" placeholder="kg" value={editWeight}
                              onChange={e => setEditWeight(e.target.value)} inputMode="decimal" className="h-8 w-24 text-sm" />
                          </div>
                          <div className="space-y-1">
                            <Label className="text-xs">体脂肪率(%)</Label>
                            <Input type="number" placeholder="%" value={editFat}
                              onChange={e => setEditFat(e.target.value)} inputMode="decimal" className="h-8 w-24 text-sm" />
                          </div>
                        </div>
                        <div className="flex gap-2">
                          ...
                          <Button size="sm" className="text-xs" onClick={handleUpdate} disabled={editLoading}>
                      ...
                      <div className="flex items-center justify-between py-2.5">
                        <div className="flex items-center gap-2.5">
                          <span className="text-sm font-medium tabular-nums">
                            {[r.weight_kg != null && `${r.weight_kg}kg`, r.body_fat_pct != null && `${r.body_fat_pct}%`]
                              .filter(Boolean).join(' · ')}
                          </span>
