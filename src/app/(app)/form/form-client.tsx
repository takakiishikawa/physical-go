...
interface Props {
  sessions: (FormSession & { exercises?: { name: string; name_ja: string } | null })[]
  ...
  return (
    <PageShell title="フォーム">
      ...
      <div className="bg-muted/40 rounded-lg border border-border p-5 space-y-4">
        {videoPreview ? (
          ...
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">トレーニング動画をアップロードするとAIがフォームを解析します</p>
            <div className="grid grid-cols-2 gap-3 max-w-sm">
              ...
              >
                ...
                <span className="text-xs font-medium text-muted-foreground">ライブラリから</span>
              </button>
            </div>
          </div>
        )}
      ...
      {sessions.length === 0 ? (
        ...
        <Section title="過去のフォームチェック" description={`${sessions.length}件`} variant="bordered">
                ...
                <Link key={session.id} href={`/form/${session.id}`}>
                  <div className="rounded-lg overflow-hidden border border-border hover:border-primary/40 hover:shadow-sm transition-all">
                    ...
                    <div className="p-2.5 space-y-1">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        {session.exercises?.name_ja && (
                          <span
                            ...
                            style={meta ? { color: meta.colorVar, borderColor: meta.colorVar + '40' } : {}}
                          >
                            {session.exercises.name_ja}
                          </span>
                        )}
                      ...
                      {fb?.overall_comment && (
                        <p className="text-[11px] text-muted-foreground line-clamp-2 leading-relaxed">{fb.overall_comment}</p>
                      )}
                    </div>
                  </div>
                </Link>
