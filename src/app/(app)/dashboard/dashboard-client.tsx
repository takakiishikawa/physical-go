...
import { ChevronRight, Video, Plus, Dumbbell, Zap, ArrowUpToLine, Trophy } from 'lucide-react'
import {
  PageHeader, SectionCards, ChartArea, Section, EmptyState,
  type KpiCard,
          ...
          <div className="flex gap-2">
            ...
            <Link href="/form">
              <Button size="sm" variant="outline" className="gap-1.5 hidden md:flex">
                <Video className="w-3.5 h-3.5" />フォームチェック
              </Button>
            </Link>
          </div>
        }
        ...
        <div className="md:col-span-2">
          <Section title="自己ベスト推移" variant="bordered">
            <div className="space-y-4 pt-2">
              {chartDataByExercise.map(({ exercise, isPullUp, data, colorVar, bestVal }) =>
                data.length > 1 ? (
                  <ChartArea
                    key={exercise.id}
                    data={data}
                    ...
                    description={bestVal !== null ? `最高: ${bestVal}${isPullUp ? '回' : 'kg'}` : undefined}
                    filterByDate={false}
                  ...
                  <Card key={exercise.id}>
                    <CardContent className="p-4 text-center">
                      <p className="text-xs text-muted-foreground">
                        {exercise.name_ja}: {data.length === 0 ? 'まだ記録がありません' : '記録が1件のみです'}
                      </p>
                    </CardContent>
                  </Card>
                )
