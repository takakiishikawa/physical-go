...
export async function POST(request: Request) {
  ...
  if (!exercises || exercises.length === 0) {
    return NextResponse.json({ error: '種目が見つかりません' }, { status: 500 })
  }

  const exerciseList = exercises.map(e => `- ${e.name}（${e.name_ja}）`).join('\n')
  ...
  const prevContext = prevFeedbacks?.length
    ? prevFeedbacks.map(f => {
        ...
        const improvements = Array.isArray(f.improvements) ? f.improvements.join('、') : ''
...
上記の種目のうちどれを行っているか判断し、フォームを分析してください。以下のJSON形式のみで返してください（コードブロックなし）：
{
  ...
  const detectedExercise = exercises.find(e => e.name === aiResponse.exercise_name) ?? exercises[0]
